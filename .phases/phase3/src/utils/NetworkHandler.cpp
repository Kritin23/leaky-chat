...
int NetworkHandler::performClientCertificateAuthentication() {
    ...
    try {
        ...
        if (!certificate.verify("certs/client/ca.crt", mHost)) {
            std::cerr << "Server certificate validation failed: time to leave." << std::endl;
            // return -1;
            exit(1);
        }
        std::cerr << "CLIENT: Server certificate validation succeeded." << std::endl;
        // exit(1);
        ...
        std::cerr << "CLIENT: Sending challenge bytes: " << challengeBuffer.size()
                  << std::endl;
        ...
        std::cerr << "CLIENT: Received server signature bytes: " << bytes_received
                  << std::endl;
        ...
        std::cerr << "CLIENT: Server proof of possession succeeded." << std::endl;
        // exit(1);
...
int NetworkHandler::performServerCertificateAuthentication() {
    ...
    std::cerr << "SERVER: Sending server certificate bytes: " << certificateBuffer.size()
              << std::endl;
    ...
    try {
        ...
        std::cerr << "SERVER: Sending signature bytes: " << signatureBuffer.size()
                  << std::endl;
